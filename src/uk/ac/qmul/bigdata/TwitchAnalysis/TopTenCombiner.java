...
public class TopTenCombiner extends Reducer<Text, Text, Text, Text> {
	...
	public void reduce(Text gameName, Iterable<Text> values, Context context)
		...
		if(testMap.firstKey() < POPULARITY_THRESHOLD){ // skipping game if the most popular channel is less than threshold
			return;
		}
		
		for(String[] recordData : testMap.values()){
			context.write(new Text(recordData[0]), new Text(recordData[0] + "\t" + recordData[1] + "\t" + recordData[2]));
		}
	}
}